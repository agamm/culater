...
function createCliSpinner(label, color = '36') {
  const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
  ...
  const render = () => {
    ...
    frameIndex += 1;
    process.stdout.write(`\r\x1b[2K\x1b[${color}m${frame}\x1b[0m ${label}`);
  };
...
<head>
  ...
  <title>culater</title>
  <style>
    :root{--bg0:#06080d;--bg1:#0f1522;--glass:rgba(15,22,34,0.75);--glass-border:rgba(255,255,255,0.14);--ink:#f1f5ff;--muted:#9ba5bd;--danger:#ff839d;--accent:#57e6cb;--login-kb-offset:0px}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body{
      ...
      padding:calc(24px + env(safe-area-inset-top)) 24px calc(24px + env(safe-area-inset-bottom));color:var(--ink);
      ...
      overflow:hidden;transition:padding-top .18s ease,align-items .18s ease
    }
    body.keyboard-open{
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    #terminal{position:relative;z-index:1;height:100%;padding:58px 10px calc(88px + env(safe-area-inset-bottom) + var(--kb-offset))}
    .xterm{
      height:100%;padding:14px;border-radius:26px;border:1px solid var(--edge);
    ...
    #reconnect{
      position:fixed;bottom:calc(84px + env(safe-area-inset-bottom) + var(--kb-offset));left:50%;transform:translateX(-50%);
    ...
    #input-dock{
      position:fixed;left:50%;bottom:calc(14px + env(safe-area-inset-bottom) + var(--kb-offset));transform:translateX(-50%);
      display:flex;align-items:center;gap:10px;z-index:130
    }
    #action-btns{
      display:flex;gap:8px;padding:8px;background:rgba(11,16,25,.78);border:1px solid var(--edge);border-radius:18px;
      backdrop-filter:blur(20px);box-shadow:0 16px 40px rgba(0,0,0,.35)
    }
    #action-btns button{
      min-width:44px;height:40px;padding:0 12px;border:1px solid rgba(255,255,255,.12);border-radius:11px;
    ...
    #cursor-toggle{
      width:46px;height:46px;border-radius:50%;border:1px solid var(--edge);background:rgba(11,16,25,.82);
      color:var(--ink);font-size:22px;line-height:1;cursor:pointer;backdrop-filter:blur(20px);
      box-shadow:0 16px 40px rgba(0,0,0,.35)
    }
    #cursor-pad{
      position:absolute;right:0;bottom:56px;width:128px;height:128px;padding:10px;border-radius:18px;
