...
async function createTunnel() {
  const localReady = await waitForReady(`http://127.0.0.1:${activePort}/healthz`, 30, 200, {
    expectedStatus: 200,
    expectedBody: 'ok'
  });
  if (!localReady) {
  ...
  tunnelProcess = spawn('cloudflared', ['tunnel', '--url', `http://localhost:${activePort}`], {
...
function tryListen(port, maxRetries = 10) {
  server.listen(port, () => {
    activePort = port;
    if (port !== Number(PORT)) console.log(`\x1b[33mPort ${PORT} in use, using ${port} instead.\x1b[0m`);
    if (DISABLE_TUNNEL) {
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    #terminal{position:relative;z-index:1;height:100%;padding:52px 8px calc(100px + env(safe-area-inset-bottom) + var(--kb-offset));overflow:hidden}
    .xterm{
    ...
    #top-bar{
      position:fixed;top:calc(6px + env(safe-area-inset-top));left:calc(8px + env(safe-area-inset-left));right:calc(8px + env(safe-area-inset-right));
      display:flex;align-items:center;justify-content:space-between;z-index:120;gap:6px
    }
    #status{
      padding:7px 14px 7px 12px;border-radius:999px;font-size:12px;line-height:1;letter-spacing:.04em;text-transform:uppercase;
      border:1px solid var(--edge);background:var(--glass);backdrop-filter:blur(18px);display:flex;align-items:center;gap:8px;
    ...
    @media (max-width:374px){#status{font-size:11px;padding:6px 10px 6px 10px;gap:6px}}
    #status::before{content:'';width:8px;height:8px;border-radius:50%;background:var(--mint);box-shadow:0 0 14px rgba(93,240,210,.65);flex-shrink:0}
    ...
    #streaming{
      ...
      background:var(--glass-2);border:1px solid var(--edge);display:none;align-items:center;gap:5px;flex-shrink:0
    }
    ...
    #top-controls{
      display:flex;align-items:center;gap:6px;padding:4px;border-radius:14px;
      background:rgba(11,16,25,.76);border:1px solid var(--edge);backdrop-filter:blur(16px);flex-shrink:0
    }
    ...
    #reconnect{
      position:fixed;bottom:calc(80px + env(safe-area-inset-bottom) + var(--kb-offset));left:50%;transform:translateX(-50%);
      padding:10px 18px;border-radius:999px;border:1px solid var(--edge);background:var(--glass-2);color:var(--ink);
      font-size:13px;cursor:pointer;display:none;z-index:131;backdrop-filter:blur(14px)
    }
    #input-dock{
      position:fixed;left:0;right:0;bottom:calc(0px + env(safe-area-inset-bottom) + var(--kb-offset));
      display:flex;align-items:flex-end;justify-content:center;gap:8px;z-index:130;
      padding:32px 10px 14px;
      background:linear-gradient(to bottom,transparent 0%,rgba(4,6,11,.95) 28%,var(--bg0) 50%)
    }
    #action-btns{
    ...
    @media (min-width:860px){
      #terminal{padding:60px 18px calc(100px + env(safe-area-inset-bottom) + var(--kb-offset))}
...
<body>
  ...
  <div id="top-bar">
    <div id="status"><span id="status-text">Connected</span></div>
